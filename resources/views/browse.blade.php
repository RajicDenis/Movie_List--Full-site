...
<div class="browse-container">
	...
	<div class="searchbar">
			<form class="search_form" action="{{ action('BrowseController@index', ['year' => $year, 'order' => $order]) }}" method="GET">			
	...
	<div class="search-body">
		...
		<div class="filter">
			...
			<div class="switch">
				<form class="switch-form" method="GET" action="{{ action('BrowseController@indexTV', ['year' => 'all']) }}">
					<input type="hidden" name="_token" value="{{ csrf_token() }}">	
						<button class="switch-btn" type="submit">Switch to TV</button>					
				</form>
			</div>
			...
			<div class="filter-options">
				...
				<div class="dropdown">
					...
					@for($i = date('Y'); $i > 1950; $i--)
					    <li class="drop-year"><a class="f-year" href="{{ action('BrowseController@index', ['year' => $i, 'order' => $order]) }}">{{ $i }}</a></li>
					@endfor
					</ul>
				</div>
				...
				<div class="dropdown">
					...
					<ul class="dropdown-menu dm2" aria-labelledby="dropdownMenu2">
						<li class="drop-genre"><a href="{{ action('BrowseController@index', ['year' => $year, 'order' => 'popdesc']) }}">Popularity<span class="glyphicon glyphicon-chevron-down chev" aria-hidden="true"></span></a></li>
						<li class="drop-genre"><a href="{{ action('BrowseController@index', ['year' => $year, 'order' => 'popasc']) }}">Popularity<span class="glyphicon glyphicon-chevron-up chev" aria-hidden="true"></span></a></li>
						<li class="drop-genre"><a href="{{ action('BrowseController@index', ['year' => $year, 'order' => 'scoredesc']) }}">Score<span class="glyphicon glyphicon-chevron-down chev" aria-hidden="true"></span></a></li>
						<li class="drop-genre"><a href="{{ action('BrowseController@index', ['year' => $year, 'order' => 'scoreasc']) }}">Score<span class="glyphicon glyphicon-chevron-up chev" aria-hidden="true"></span></a></li>
					</ul>
				</div>
		...
		<div class="filter-body">
			<div class="filter-body1">
				...
				<div class="f-yearbox">
					@if($year != 'all') 
						...
						<a href="{{ action('BrowseController@index', ['year' => 'all', 'order' => $order]) }}" class="glyphicon glyphicon-remove red remove_year"></a>
					@endif
				</div>
				...
				<div class="f-yearbox">
					@if($order != null) 
						...
						<a href="{{ action('BrowseController@index', ['year' => $year, 'order' => null]) }}" class="glyphicon glyphicon-remove red remove_year"></a>
					@endif
				</div>
			...
			@if(count($movies) != 0)
				...
				<div class="browse_pgn">{!! $movies->links() !!}</div>

			@else
				<h3 style="font-family: 'Amaranth', sans-serif;">No Movies added yet!</h3>
			@endif
